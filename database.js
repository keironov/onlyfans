...
export function addReport(username, message, date){
  ...
  db.prepare("INSERT INTO reports(username,message,date,task_type,suspicious) VALUES(?,?,?,?,?)")
    .run(username, message, date, task_type, suspicious ? 1 : 0);
  ...
  if(types[task_type] !== undefined) types[task_type]++;

  db.prepare("UPDATE users SET total_reports=?, avg_length=?, types_json=?, last_report=? WHERE username=?")
...
function updateActivityHeat(username, dateStr){
  ...
  if(row){
    db.prepare("UPDATE activity_heat SET count=count+1 WHERE username=? AND day=? AND hour=?").run(username,day,hour);
  } else {
    db.prepare("INSERT INTO activity_heat(username,day,hour,count) VALUES(?,?,?,1)").run(username,day,hour);
  }
}
...
// ------------------ Получение данных ------------------
export function getUsers(){ return db.prepare("SELECT * FROM users").all(); }
export function getReports(){ return db.prepare("SELECT * FROM reports ORDER BY date DESC LIMIT 100").all(); }
export function getUserMetrics(username){ return db.prepare("SELECT * FROM users WHERE username=?").get(username); }
export function getActivityHeat(){ return db.prepare("SELECT * FROM activity_heat").all(); }
